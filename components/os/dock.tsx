...
function DockItem({
  ...
  return (
    <button type='button' onClick={() => onOpen(appId)} className='group relative flex flex-col items-center' aria-label={`Open ${meta.name}`}>
  ...
  return (
    <div className='pointer-events-none absolute inset-x-0 bottom-0 flex justify-center pb-2.5' style={{ zIndex: 9000 }}>
      <div className='os-panel pointer-events-auto flex items-end gap-2 rounded-[20px] border px-3 py-2.5 shadow-xl sm:gap-2.5'>
        ...
        <DockItem appId='trash' open={openSet.has('trash')} bouncing={bouncing.has('trash')} onOpen={handleOpen} reduce={reduce} />
      </div>
    </div>
  );
}
